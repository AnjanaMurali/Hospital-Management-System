...
printf("\n\t\t\t\t3. Edit Patients Record\n"); 
printf("\n\t\t\t\t4. Know the record of thepatients\n");
30
...
35
fprintf(ek,"%d%s %c %i %s %s %s %s %s\n",p.number,p.First_Name, p.Gender, 
...
=========================================================");
row=17;
while(fscanf(ek,"%s %c %i %s %s %s %s %s\n", p.First_Name, &p.Gender, &p.age, 
p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor)!=EOF)
{
gotoxy(1,row); 
...
while(fscanf(ek,"%d %s %c %i %s %s %s %s %s\n", 
p.number,p.First_Name,&p.Gender, &p.age, p.Address, p.Contact_no, p.Email, 
...
gets(name); 
name[0]=toupper(name[0]);
while (fscanf(ek,"%d %s %c %i %s %s %s %s %s", p.number,p.First_Name, 
&p.Gender,&p.age, p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor)!=EOF)
{
if (strcmp(p.First_Name,name)!=0)
fprintf(ft,"%d,%s %c %i %s %s %s %s %s\n",p.number, p.First_Name, p.Gender, 
...
{
printf("%d %s %c %i %s %s %s %s %s\n", p.number,p.First_Name, p.Gender, p.age, 
p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor);
found=1;
}
}//while loop ends 
if(found==0)
